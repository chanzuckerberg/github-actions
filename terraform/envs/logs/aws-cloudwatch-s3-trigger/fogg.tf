...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::724328632434:role/tfe-si"
  }

  allowed_account_ids = ["724328632434"]
}
...
terraform {
  ...
  backend "remote" {
    ...
    workspaces {
      name = "logs-aws-cloudwatch-s3-trigger"
    }
...
variable "component" {
  type    = string
  default = "aws-cloudwatch-s3-trigger"
}
variable "owner" {
  type    = string
...
variable "tags" {
  ...
  default = {
    ...
    env       = "logs"
    service   = "aws-cloudwatch-s3-trigger"
...
data "terraform_remote_state" "splunk-ingest" {
  backend = "remote"
  config = {
